 ...
 * Main JavaScript
 *
 * Page loading, section scrolling, portfolio and other main features are handled by this.
 */

jQuery(document).ready(function($){

	var settings_zp = {"is_home":"1"};
	var multilang_pp = {"prev":"Prev","next":"Next","expand":"Expand","close":"Close"};
	...
	$('#portfolio-controls').prepend('<div id="loading-screen"></div>');
	...
	var max_thumb_rows = 5; // items causing additional rows trigger pagination

	// Hide category filters if there is only one category (or none)
	var category_controls_hidden = false;
	if ($('#portfolio-filters li').size() <= 2) { // 2 because "All" is always present
	...
	$().prettyPhoto({ // fire it up
		...
		// lightbox config
		opacity: .9, // we hide page content (much better performance), revealing background instead
		...
		markup: '<div class="pp_pic_holder"> \
					<div class="pp_content_container"> \
						<div class="pp_content"> \
							...
							<div class="pp_fade"> \
								<a href="#" class="pp_expand" title="' + multilang_pp.expand + '">' + multilang_pp.expand + '</a> \
								<div class="pp_hoverContainer"> \
									<a class="pp_next" href="#">' + multilang_pp.next + '</a> \
									<a class="pp_previous" href="#">' + multilang_pp.prev + '</a> \
								</div> \
								...
								<div class="pp_details"> \
									...
									<div class="pp_nav"> \
										...
										<a href="#" class="pp_arrow_previous">' + multilang_pp.prev + '</a> \
										<a href="#" class="pp_arrow_next">' + multilang_pp.next + '</a> \
										<a class="pp_close" href="#">' + multilang_pp.close + '</a> \
									</div> \
	...
	/* ADD PORTFOLIO LINK */
	$('li .dblink').click(function(){
		addr = $(this).attr('data-href'); 
		window.location = addr;
	});
	/* END ADDING PORTFOLIO LINK */

	$('.portfolio-thumb-overlay').each(function () {
		titset = $(this).parent('').parent().attr('data-categories');
		$(this).children('h2').children('small').attr('title', titset)
	})

	$(".portfolio-thumbs").on('click', 'a', function(event) { //$(".portfolio-thumbs").delegate('a', 'click', function(event) {	 ; SUSPENDED IN 1.7
		event.preventDefault(); // stop normal click action
		$.prettyPhoto.open(portfolio_filtered_urls, portfolio_filtered_titles, portfolio_filtered_descs); // build arrays of image data from filtered master list
		$('#header, #footer, #sections section, #color-switcher').hide(); // hide page content, showing background only - much better performace especially in Chrome
		...
		$(this).children('.dblink').click(function(){
			event.preventDefault();
		});
	});
	...
	// do this only if controls were not hidden due to category shortage
	if (!old_ie && !category_controls_hidden) { // this is ugly for IE 7/8
	...
	var start_section_id = window.location.hash && $(window.location.hash).length ? window.location.hash.replace('#', '') : $('#sections > section:first-child').attr('id'); // if no existing section given in hash, use first section
		...
		if (phase == 2 || !phase) {
			...
			if ($('.section-portfolio').length) {
				...
				setTimeout(function() {		
					...
					if (!category_controls_hidden) { // do this only if controls are not hidden due to category shortage
	...
	$("body").imageLoader({
		...
		// Slide in effect
		var scrollable = $(document).height() > $(window).height() + 50; // if document height greater than viewport, scrolling can be done (+50 is so scrolling can be seen)
		if (settings_zp.is_home && !no_scrolling_effects && scrollable) { // also must be home (no scroll in on blog and 404)
		...
		// No slide in effect (mobile browsers or if document height less than viewport - not scrollable)
		// Or if this is non-homepage (external blog)
		else {
	...
	// Scroll via clicked link
	$('a.smoothscroll').click(function(event) { // $('a').smoothScroll() version doesn't work well with Opera
		if ($(this).attr('id') != 'cancel-comment-reply-link') {
			smooth_scroll($(this).prop('href'), event);	// use prop for absolute URL, not attr		
		}
	});
	

	
	// Do smooth scroll (either from click an a link or hash preset at load on non-homepage)
	function smooth_scroll(absolute_url, click_event) {
		// get hash from URL
		var hash = '';
		var index = absolute_url.indexOf('#');
		if (index > 0) {
			hash = absolute_url.substr(index);
		}
		// URL has hash and URL minus hash is same as current page's URL minus hash, and has corresponds to existing element to scroll to
		// this makes it so the menu will work when on external page (blog, 404) as well as URL's to external sites that have hashes
		if (hash != '' && hash != '#' && $(hash).length > 0 && same_page(absolute_url)) {
			// Smooth scroll...
			if (!no_scrolling_effects) {
				// clicked a link
				if (click_event) {
					click_event.preventDefault(); // prvent regular action
				}
			
				// don't let menu/scroll work until top section scrolls in (does not apply to non-homepage)
				if (loading_scroll_done || !settings_zp.is_home) {
					...
					$.smoothScroll({
						...
						afterScroll: function() {
							if (settings_zp.is_home && ie != 7) { // homepage sections only because non-homepage adds offset for menu; IE7 can't handle this nicely
	...
	// Determine if given URL is same as current URL when hash is not considered
	function same_page(absolute_url) {

		var cur_base_url = url_remove_hash(window.location.href); // remove any hash
		var dest_base_url = url_remove_hash(absolute_url); // remove any hash

		if (cur_base_url == dest_base_url) {
			return true;
		}
	...
	// Remove hash from URL if it is present
	function url_remove_hash(url) {
